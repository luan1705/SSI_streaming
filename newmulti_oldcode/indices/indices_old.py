...
def on_message_MI(message):
    try:
        ...
        #Trong giờ giao dịch VN [09:00, 15:00) -> thêm close = IndexValue (điểm hiện tại)
        local_now = datetime.now(ZoneInfo("Asia/Ho_Chi_Minh")).time()
        if dtime(9, 0) <= local_now < dtime(15, 0):
            row["close"] = float(c.get("point"))           
