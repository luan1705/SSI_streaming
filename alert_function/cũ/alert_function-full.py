...
def preload_history_cache():
    ...
    for key in r.scan_iter("ohlcv:*"):
        try:
            symbol = key.split("ohlcv:")[-1]
            history_cache[symbol] = get_data_from_redis(symbol)
            loaded += 1
        except Exception as e:
            print(f"Preload lỗi {key}: {e}")

    print(f"Preloaded {loaded} symbols into history_cache")
    ...
    while True:
        try:
            ...
            for symbol, data in latest_by_symbol.items():
                ...
                (
                    df, EMA10, EMA20, EMA50, SMA10, SMA20, SMA50,
